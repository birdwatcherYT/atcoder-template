...
bool can_go(const tuple<int,int,int> &current, const tuple<int,int,int> &dist){
	int t=get<0>(dist)-get<0>(current);
    int x=get<1>(dist)-get<1>(current);
    int y=get<2>(dist)-get<2>(current);
